...
class TestPydanticSchemas(TestCase):
    ...
    def test_string_normalization(self):
        ...
        data_with_whitespace = {
            'external_id': '  test_123  ',  # Whitespace
            'source': 'json',
            'name': '  Test POI  ',  # Whitespace
            'latitude': Decimal('40.7128'),
            'longitude': Decimal('-74.0060'),
            'category': '  restaurant  ',  # Whitespace
            'description': '  Test description  '
        }
        
        validated = validate_poi_record(data_with_whitespace, 'test_file')
        
        # Strings should be stripped
        self.assertEqual(validated.external_id, 'test_123')
        self.assertEqual(validated.name, 'Test POI')
        self.assertEqual(validated.category, 'restaurant')
